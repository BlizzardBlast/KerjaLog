...
describe('getOrCreateDatabaseKey', () => {
  ...
  test('returns an existing valid database key without rotating it', async () => {
    ...
    await expect(getOrCreateDatabaseKey()).resolves.toBe(storedKey);

    expect(getRandomBytesAsyncMock).not.toHaveBeenCalled();
    expect(setItemAsyncMock).not.toHaveBeenCalled();
  });

  test('rejects a corrupt stored key instead of generating a replacement', async () => {
  ...
  test('generates and stores a 256-bit hexadecimal key when none exists', async () => {
